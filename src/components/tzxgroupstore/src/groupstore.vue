...
<script>
	export default{
					...
					arrays.map(item => {
						if(item.id == it) {
							...
							if(!item.children ) {
								...
								this.findNextItemNode(item.children,it);
							}
						}
					})
			},
		...
		mounted(){ 
			this.defaultCheckedId.forEach(item =>{
				this.findNextItemNode(this.data,item)
			})
			this.$emit('renderStore',this.nodes);
			//this.ruleForm.mendian = this.nodes;
			if(this.isDisabled){

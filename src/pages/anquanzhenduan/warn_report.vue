<template>
	<div class='applist' v-loading.body='loadingBody'>
		...
		<div class='reportdetail'>
			...
			<div class='reportlist'><em>报告类别&nbsp;:</em><span>消费信息诊断报告</span></div>
				    ...
				    <el-table-column  header-align='center' align='center'  width="200">
					   ...
					   <el-table-column type="expand"  width="410">					   
     					 <template scope="props">
							    ...
							    <el-table-column align='right' header-align='right' label="次数"  width="80" v-if='props.row.warnName.indexOf("次数")!= -1'>
							    	<template scope="scope" >										
							...
							<!--    <el-table-column header-align='right' align='right'   prop="warn_real" :label="setTitleName(props.row.detailList)" width="150"></el-table-column>-->
								...
								<el-table-column align='right' header-align='right'   label="主账户金额" width="100" v-if='props.row.warnName.indexOf("储值")!= -1'>	
									<template scope="scope" >										
									<div class='memberdetail' v-for="item in scope.row.detailVoList">
										{{item.main_trading | currency}}
									</div>
									</template>
								</el-table-column>
								<el-table-column align='right' header-align='right'   label="赠送账户金额" width="120" v-if='props.row.warnName.indexOf("储值")!= -1'>	
									<template scope="scope" >										
									<div class='memberdetail' v-for="item in scope.row.detailVoList">
										{{item.reward_trading | currency}}
									</div>
									</template>
								</el-table-column>
								<el-table-column align='right' header-align='right'   label="积分" width="100" v-if='props.row.warnName.indexOf("积分")!= -1'>	
									<template scope="scope" >										
									<div class='memberdetail' v-for="item in scope.row.detailVoList">
										{{item.credit}}
									</div>
								...
								<el-table-column  align='right' header-align='right'   label="储值时间" width="90" v-if='props.row.warnName.indexOf("消费") == -1 && props.row.warnName.indexOf("增加") == -1'>	
									<template scope="scope" >										
									<div class='memberdetail'  v-for="item in scope.row.detailVoList">
										{{item.operate_time}}
									</div>
									...
									<el-table-column  align='right' header-align='right'   label="消费时间" width="90" v-if='props.row.warnName.indexOf("消费")!= -1'>	
									<template scope="scope" >										
									<div class='memberdetail'  v-for="item in scope.row.detailVoList">
										{{item.operate_time}}
									</div>
									...
									<el-table-column  align='right' header-align='right'   label="增加时间" width="90" v-if='props.row.warnName.indexOf("增加")!= -1'>	
									<template scope="scope" >										
									<div class='memberdetail'  v-for="item in scope.row.detailVoList">
										{{item.operate_time}}
									</div>
								...
								<el-table-column align='left' header-align='left' label="操作员" width="80">	
									<template scope="scope" >										
									<div class='memberdetail' v-for="item in scope.row.detailVoList">
										{{item.operator}}
									</div>
...
/*	import { getReportById} from '@/api/article'*/
	import { getReportById} from '@/api/activity'
	...
	export default{
		...
		methods:{
			...
			findReportDetail(day_count){
				...
				getReportById(message).then(reponse=>{
					if(reponse.success){

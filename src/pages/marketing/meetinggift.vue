<template>
	<div class="context" v-loading.body="loading1">
		...
		<!--消费后评价赠礼  开始-->
		<div class="divTwo_rhzl">
			<h1><em>入会赠礼</em><cite>为第一次注册成为会员的顾客赠送优惠</cite></h1>
...
<script>
	...
	import { findMeetingGift, findMeetingSmsTemplate, findMeetinggiftActivityDetail,saveMeetingGift } from '@/api/activity'
	import axios from "axios";

	export default {
		components:{TzxCouponSelect,TzxCouponView,TzxTreeSelect,TzxTag},
		data() {
			return {
				///111111111
		...
		methods: {
			...
			onSubmit(form) {
				...
				this.$refs[form].validate((valid) => {
					if(valid) {
						...
						saveMeetingGift(data).then((response) => {
							this.loading1 = false;
							if(response.success ) {
								this.$message({
									type: 'success',
			...
			loadFindSmsTemplate() {
				findMeetingSmsTemplate().then((response) => {
				//findSmsTemplate().then((response) => {
					this.message = response;
				})
			},
			loadFindEvalGift() {
				findMeetingGift().then((response) => {
			...
			openDialogQunti: function(e) { //打开会员群体，发送会员群体请求
				this.dialogQuntiVisible = true;
				this.$nextTick(function() {
					if(this.$refs.mytable != undefined) {
						this.toggleSelection(this.tableDataFilter);
					}
				})
			},
			...
			updateFormData(id, activity_type) {
				...
				findMeetinggiftActivityDetail(query).then(response => {
	...
	created() {
		_czc.push(["_trackEvent", "营销活动-入会赠礼", "新增编辑"]);//埋点统计
		this.loadTree();
...
<style rel="stylesheet/scss" lang="scss">
	...
	.divTwo_rhzl .demo-ruleForm .hdzq_date1 {
		float: left;
	}
	
	.divTwo_rhzl .demo-ruleForm .hdzq_date_em {
		float: left;
		margin-left: 5px;
		margin-right: 5px;
		font-style: normal;
	...
	.divTwo_rhzl .demo-ruleForm .hdzq_date2 {
		float: left;
	}
	/******表单样式 开始******/
	...
	.divTwo_rhzl .el-form .el-form-item:nth-last-child{
		margin-top: 30px;
	}
	
	.divTwo_rhzl .el-date-editor .el-date-editor--date {
		width: 230px;
	}
	/***活动赠礼***/
	...
	.divTwo_rhzl{
		.divTwo_rhzl .cyqt_person_num h1,
		.cyqt_person_num tt,
	...
	.divTwo_rhzl .el-form .el-form-item__error {
		margin-left: 40px;
	}
	.divTwo_rhzl .el-form .el-form-item .el-form-item .el-form-item__error {
		margin-left: -2px;
	}
	.divTwo_rhzl .yhq_hdzl .el-form-item__content {
		margin-left: 20px !important;
	}
	.divTwo_rhzl .yhq_hdzl .el-form-item__error {
	...
	.divTwo_rhzl .pjzl_hdzl .txsj .el-radio__label {
		color: #243546;
	}
	
	.divTwo_rhzl .pjzl_hdzl .txsj cite {
		color: #9AABB8;
	}
	
	.divTwo_rhzl .pjzl_hdzl .txsj .el-radio {
		margin-left: 92px;
	}
	
	.divTwo_rhzl .pjzl_hdzl .el-form-item:nth-child(n+1) .el-form-item__content {
		line-height: 20px;
	}
	...
	.divTwo_rhzl .hdtx_bu_weixin .el-checkbox-group .el-checkbox:nth-child(3) {
		margin-left: 50px;
	}
	
	.divTwo_rhzl .hdtx_bu_weixin .el-checkbox-group .el-checkbox:nth-child(1) {
		float: left;
	}
	
	.divTwo_rhzl .hdtx_bu_weixin .el-checkbox-group .el-checkbox:nth-child(2) {
		clear: both;
	}
	
	.divTwo_rhzl .hdtx_bu_weixin .el-checkbox-group cite {
		display: block;
		color: #9AABB8;
		width: 400px;
		line-height: 32px
	...
	.divTwo_rhzl .hdtx_bu_weixin .el-form-item__content {
		margin-left: 20px!important;
	}
	...
	.divTwo_rhzl .hdtx_bu_weixin .el-form-item__content .el-checkbox-group .el-checkbox:nth-child(4) {
		clear: both;
	}
	
	.divTwo_rhzl .hdtx_dxtx {
		/* background: #F5F9FB; */
		border-radius: 3px;
		margin-right: 40px;
		/* overflow: hidden; */
	...
	.divTwo_rhzl .hdtx_dxtx .dxmb_text {
		/*width: 599px;*/
		float: left;
		height: 50px
	}
	
	.divTwo_rhzl .hdtx_dxtx .dxmb_text cite {
		display: block;
		line-height: 50px;
	}
	
	.divTwo_rhzl .hdtx_dxtx .dxmb_text em {
		clear: both;
		color: #0C9AFF;
	}
